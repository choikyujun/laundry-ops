...
for (const funcName of functionsToDeduplicate) {
    ...
    if (indices.length > 1) {
        ...
        for (let i = 0; i < indices.length - 1; i++) {
            ...
            // Replace e.g., "window.viewSentDetail = async function" with "window.OLD_viewSentDetail_0 = async function"
            let fnVar = funcName.split(' = ')[0];
            let rest = funcName.split(' = ')[1];
            newCode += fnVar.replace('window.', 'window.OLD_') + '_' + i + ' = ' + rest + ' (';
