...
let regexAdminList = /let query = window\.mySupabase[\s\S]*?\.eq\('factory_id', currentFactoryId\)\n\s*\.not\('staff_name', 'like', '관리자\(차감\)%'\);/;
...
let regexAdminStaffList = /const { data: invoices, error: iErr, count } = await window\.mySupabase\.from\('invoices'\)\s*\.select\('\*, hotels\(name\)', { count: 'exact' }\)\s*\.eq\('factory_id', currentFactoryId\)\s*\.not\('staff_name', 'like', '관리자\(차감\)%'\)/;
