...
window.sendInvoicesToClient_OLD_1 = function() {
    console.log('발송 시작');
    if(!window.checkInvoiceFilters()) { alert('필수 항목을 모두 선택해주세요.'); return; }
...
window.updateHotelItemPrice = async function(id, newPrice) {
    ...
    // 수정 후 목록 갱신을 위해 UI 갱신 (단, 현재 보고 있는 탭에 따라 다름)
    // 리스트를 완전히 다시 불러오지 않고, 현재 row의 값만 갱신하거나 
    // 최소한 다른 항목들이 삭제되지 않도록 합니다.
...
window.openInvoiceModal = async function() {
    ...
    let itemsToRender = [];
    // 거래처 전용 단가 가져오기
    // [v38 수정] 정렬 순서 적용 (sort_order -> created_at)
    const { data: hItemsData, error: hErr } = await window.mySupabase
    ...
    if (hItemsData && hItemsData.length > 0) {
        itemsToRender = hItemsData;
    } else {
        const { data: fData } = await window.mySupabase.from('factories').select('default_items').eq('id', currentFactoryId).single();
        if (fData && fData.default_items) {
            itemsToRender = fData.default_items;
...
window.sendInvoicesToClient_DISABLED = async function() {
    if(!window.checkInvoiceFilters()) { alert('필수 항목을 모두 선택해주세요.'); return; }
...
window.sendInvoicesToClient_DISABLED = async function() {
    ...
    document.getElementById('sendInvBtn').onclick = async function() {
        console.log("DEBUG: 발송 버튼 클릭됨");
        if(confirm(`${h.name} 거래처로 정산명세서를 발송하시겠습니까?`)) {
            console.log("DEBUG: 사용자가 발송 확인함");
            try {
                // 1. 발송 상태 업데이트
                const ids = list.map(inv => inv.id);
                console.log("DEBUG: Updating invoices:", ids);
                await window.mySupabase.from('invoices').update({ is_sent: true }).in('id', ids);
                
                // 2. sent_logs 테이블에 발송 기록 추가 (에러 체크 포함)
                const sentLogData = {
                    factory_id: currentFactoryId,
                ...
                alert("DEBUG: Sending to DB: " + JSON.stringify(sentLogData));
                const { error: logErr } = await window.mySupabase.from('sent_logs').insert([sentLogData]);
                
                if (logErr) {
                    console.error("DEBUG: sent_logs insert error:", logErr);
                    throw logErr;
                }
                
                console.log("DEBUG: sent_logs insert successful");
                alert('발송이 완료되었습니다.');
                window.loadAdminRecentInvoices(); 
                window.loadAdminSentList(); // 발송 내역 리스트 갱신
                closeModal('sendInvoiceModal');
            } catch (e) {
                alert('발송 기록 저장 중 오류 발생: ' + e.message);
                console.error("DEBUG: 발송 전체 에러:", e);
            }
        }
    };
...
window.addSimpleItem = async function() {
    ...
    const payload = {
        factory_id: currentFactoryId,
        hotel_id: hId,
        category_id: cat.id, // [v38 수정] 카테고리 ID 포함
        category_name: '기본', // [v38 수정] 카테고리명 포함
        name: name,
        price: price,
        unit: unit,
        sort_order: 999 // 기본값
    ...
    if (error) console.error("DEBUG: addSimpleItem error:", error);
...
window.mySendInvoices = async function() {
    ...
    openModal('sendInvoiceModal');
};window.sendInvoicesToClient_V38 = async function() {
    console.log('DEBUG: mySendInvoices (Matrix UI) 시작');
    if(!window.checkInvoiceFilters()) { alert('필수 항목을 모두 선택해주세요.'); return; }
