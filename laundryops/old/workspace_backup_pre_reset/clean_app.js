...
window.switchTab = async function(el, tabId) {
    ...
    if(tabId === 'adminStats') {
        ...
        if(typeof window.loadAdminRecentInvoices === 'function') window.loadAdminRecentInvoices(); // 목록 직접 호출!
    }
...
/* removed dup */
window.loadAdminDashboard = function() {
    if(typeof originalLoadAdminDashboard === 'function') {
        originalLoadAdminDashboard(); // 기존 UI(매출 0원, 통계 등 껍데기) 그리기
    }
    // 진짜 핵심: 하단 명세서 리스트 DB에서 불러와서 뿌리기!
...
window.switchTab = async function(el, tabId) {
    ...
    if(tabId === 'adminStats') {
        ...
        // [중요] 대표 대시보드 탭을 누르면, 무조건 DB에서 거래명세서 목록을 긁어옵니다!
...
const _originalLoadAdminDashboard = window.loadAdminDashboard;
/* removed dummy */
...
window.showView = async function(id, title) {
    ...
    // 대표 화면으로 전환되면 무조건 DB에서 명세서를 긁어온다!
    if (id === 'adminView') {
        setTimeout(async () => {
            if(typeof window.loadAdminRecentInvoices === 'function') {
                await window.loadAdminRecentInvoices();
            }
        }, 100); // UI가 렌더링된 후 0.1초 뒤에 긁어오기 (안전빵)
    }
};
...
window.loadSuperAdminDashboard = async function() {
    ...
    // (결제 승인 모듈은 v34 범위 외라면 기존 방식 유지 또는 추후 개발)
    ...
    // 1. Factories 테이블에서 모든 공장 가져오기
    const { data: factories, error: fErr } = await window.mySupabase
        .from('factories')
        .select('*');

    if (fErr || !factories) {
        tbody.innerHTML = '<tr><td colspan="6" style="text-align:center; color:red;">공장 데이터를 불러올 수 없습니다.</td></tr>';
        return;
    }

    if (factories.length === 0) {
        tbody.innerHTML = '<tr><td colspan="6" style="text-align:center;">가입된 공장이 없습니다.</td></tr>';
        return;
    }

    // 2. Invoices 테이블에서 해당 월(curMonth) 매출만 그룹화해서 가져오기 (PostgREST의 RPC나 View 없이 클라이언트 썸)
    // - 실무에서는 DB 뷰(View)를 만들지만, 지금은 간단하게 처리
    const startDate = curMonth + '-01';
    const endDate = curMonth + '-30'; // or use end of month calculation. Let's just do 31 but catch invalid dates // 대략적으로 월말

    const { data: invoices, error: iErr } = await window.mySupabase
        .from('invoices')
        .select('factory_id, total_amount')
        .gte('date', startDate)
        .lte('date', endDate);

    // 공장별 이번 달 명세서(단가제) 총합 계산
    const monthlyRevMap = {};
    if (invoices) {
        invoices.forEach(inv => {
            if(!monthlyRevMap[inv.factory_id]) monthlyRevMap[inv.factory_id] = 0;
            monthlyRevMap[inv.factory_id] += (inv.total_amount || 0);
        });
    }

    // 3. 정액제 거래처 매출 합산
    const { data: fixedHotels, error: hErr } = await window.mySupabase
        .from('hotels')
        .select('factory_id, fixed_amount')
        .eq('contract_type', 'fixed');

    if (fixedHotels) {
        fixedHotels.forEach(h => {
            if(!monthlyRevMap[h.factory_id]) monthlyRevMap[h.factory_id] = 0;
            monthlyRevMap[h.factory_id] += (h.fixed_amount || 0);
        });
    }
    ...
    factories.forEach(f => {
        // 검색 필터링 (가입 대기 상태인 'pending'은 마스터 대시보드 메인 목록에서 제외)
        if (f.status === 'pending') return;
        if (searchQuery && !f.name.toLowerCase().includes(searchQuery)) return;

        totalFactories++;
        if (f.created_at && f.created_at.startsWith(curMonth)) newFactories++;

        const monthlyRevenue = monthlyRevMap[f.id] || 0;
        totalRev += monthlyRevenue;
        factorySales.push({ name: f.name, revenue: monthlyRevenue });

        const subBadgeColor = f.sub_status === 'expired' ? '#ef4444' : (f.sub_status === 'expiring' ? '#f59e0b' : '#10b981');
        let subLabel = '활성(사용중)';
        if(f.sub_status==='expired') subLabel='만료됨'; else if(f.sub_status==='expiring') subLabel='만료임박'; else if(f.sub_status==='trial') subLabel='무료체험';

        const subBadge = `<span style="background:${subBadgeColor}; color:white; padding:2px 8px; border-radius:12px; font-size:11px;">${subLabel}</span>`;
        
        const statusSelect = `<select onchange="updateFactoryStatus('${f.id}', this.value)" style="background:${f.status==='suspended'?'#94a3b8':'#00a8e8'}; color:white; border:none; padding:5px; border-radius:4px; font-size:12px;">
            <option value="operating" ${f.status==='operating'?'selected':''}>운영중</option>
            <option value="suspended" ${f.status==='suspended'?'selected':''}>미운영</option>
        </select>`;
        
        const rowStyle = f.status === 'suspended' ? 'style="background-color: #f1f1f1;"' : '';
        
        tbody.innerHTML += `<tr ${rowStyle}>
            <td title="${monthlyRevenue.toLocaleString()}원"><strong>${f.name}</strong></td>
            <td>${f.admin_id}</td>
            <td>${statusSelect}</td>
            <td>${subBadge}</td>
            <td>${f.plan || '무료요금제'} / ${f.plan_expiry || '-'}</td>
            <td>
                <!-- 수정 팝업 구현은 v34 후반부나 v35에서! 일단 삭제만 -->
                <button class="btn btn-danger" style="padding:5px; font-size:12px; border-radius:4px; border:none;" onclick="deleteFactory('${f.id}')">삭제</button>
            </td>
        </tr>`;
    ...
    if (totalFactories === 0) {
        tbody.innerHTML = '<tr><td colspan="6" style="text-align:center;">운영 중인 공장이 없습니다.</td></tr>';
    }
    ...
    if(rankArea) {
        ...
        factorySales.sort((a,b) => b.revenue - a.revenue).slice(0, 10).map((fs, i) => `
            <tr><td>${i+1}위</td><td>${fs.name}</td><td style="text-align:right; font-weight:700; color:var(--primary);">${fs.revenue.toLocaleString()}원</td></tr>
        `).join('') + '</tbody></table>';
    }
};
...
window.deleteFactory = async function(fId) {
    if(confirm('정말 이 세탁공장을 삭제하시겠습니까? 관련된 모든 데이터(거래처, 명세서)가 연쇄적으로 영구 삭제됩니다!')) {
        await window.mySupabase.from('factories').delete().eq('id', fId);
        window.loadSuperAdminDashboard();
    }
};
...
// [v34 찐마지막] 탭 클릭이나 로그인 직후에 아무것도 못 믿겠으니 무식하게 때려박습니다.
window.addEventListener('hashchange', () => {
    if(window.location.hash !== '#superadmin' && document.getElementById('adminRecentInvoiceList')) {
        setTimeout(() => { if(typeof window.loadAdminRecentInvoices === 'function') window.loadAdminRecentInvoices(); }, 500);
    }
});

// 로그인 버튼에서 호출되는 showView 함수 가로채서 무조건 호출!
const _realShowView = window.showView;
window.showView = function(id, title) {
    if (typeof _realShowView === 'function') _realShowView(id, title);
    if (id === 'adminView') {
        console.log("DEBUG: Force-loading admin dashboard and invoices from showView!");
        setTimeout(() => {
            if(typeof window.loadAdminDashboard === 'function') window.loadAdminDashboard();
            if(typeof window.loadAdminRecentInvoices === 'function') window.loadAdminRecentInvoices();
        }, 500);
