...
// The regex replace in the previous script failed or did not match perfectly.
// Let's manually replace the block in patch_sent_group_monthly.js

const oldA = `        // DB에서 최신 발송 내역과 조합하여 "발송 그룹"을 묶어내기 (과거 로컬 데이터가 날아갔을 경우를 대비)
...
const newA = `        // 1. 로컬 스토리지에서 실제 발송(버튼 클릭) 기록 가져오기 (여기에 2026-04-01 ~ 2026-04-11 이 박혀있음)
        let f = null;
        if (typeof platformData !== 'undefined' && platformData.factories) {
            f = platformData.factories[currentFactoryId];
        } else {
...
if(codeA.includes('// DB에서 최신 발송 내역과 조합하여')) {
    codeA = codeA.replace(oldA, newA);
    fs.writeFileSync('patch_sent_group_monthly.js', codeA);
    console.log("Replaced admin list logic");
}


// Now partner list logic
let codeP = fs.readFileSync('patch_partner_dashboard_v35.js', 'utf8');

const oldP = `        // 100% DB Driven
...
const newP = `        // 1. 로컬에서 진짜 조회 기간(period) 가져오기
        let f = null;
        if (typeof platformData !== 'undefined' && platformData.factories) {
            f = platformData.factories[currentFactoryId];
        } else {
