...
async function initApp() {
    ...
    // 관리자가 특정 공장으로 바로가기를 시도한 경우
    const adminAccessId = localStorage.getItem('adminAccessFactoryId');
    if (adminAccessId) {
        const { data: f } = await window.mySupabase.from('factories').select('name').eq('id', adminAccessId).maybeSingle();
        if (f) {
            currentFactoryId = adminAccessId;
            localStorage.setItem('currentFactoryId', adminAccessId);
...
async function syncToSupabase(data) {
    if (!window.mySupabase) return;
    try {
        await window.mySupabase.from('platform_data').upsert({ id: 1, data: data });
...
// --- Admin Credentials & Global Notice Logic ---
...
window.saveNotice = async function() {
    ...
    platformData.globalNotice = noticeContent;
    await window.saveData(); // Supabase platform_data JSON 저장
    alert(noticeContent ? '공지사항이 등록/업데이트 되었습니다.' : '공지사항이 삭제되었습니다.');
    window.loadGlobalNotice();
};

window.loadGlobalNotice = function() {
    const bar = document.getElementById('globalNoticeBar');
    const input = document.getElementById('globalNoticeInput');
    
    const notice = platformData && platformData.globalNotice ? platformData.globalNotice : '';
...
window.openFactoryAdminView = function(fId) {
    ...
    if (!currentPath || currentPath === '') currentPath = '거래명세서프로그램v35.html';
...
window.loadHotelReceivedInvoicesList = function() {
    ...
    myInvoices.sort((a,b) => new Date(b.sentAt) - new Date(a.sentAt)).forEach(inv => {
        ...
        // [v36 수정] 기간 표시를 YYYY-MM 형태로 보기 좋게 변환
...
window.confirmSendInvoice = async function(sDate, eDate, hotelId, totalAmount, supplyPrice, vat) {
    ...
    if (f && f.history) {
        ...
        await saveData(); // [v36 수정] Supabase JSON blob에 저장
        alert('성공적으로 발송되었습니다.');
...
window.loadAdminDashboard = async function() {
    console.log("DEBUG: [v35] loadAdminDashboard DB version started");
