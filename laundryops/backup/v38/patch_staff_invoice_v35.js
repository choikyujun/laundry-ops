window.openInvoiceModal = async function() {
    ...
    let { data: itemsToRender } = await window.mySupabase.from('hotel_item_prices').select('*').eq('hotel_id', hId).order('category_name').order('created_at');
    
    // 만약 품목이 없으면 factory_default_prices 에서 대체로 가져오기
    if (!itemsToRender || itemsToRender.length === 0) {
        const { data: fData } = await window.mySupabase.from('factory_default_prices').select('*').eq('factory_id', currentFactoryId);
...
window.calcTotal = function() {
    ...
    document.getElementById('invoiceTotalDisplay').innerText = total.toLocaleString() + '원';
};

window.saveStaffInvoice = async function() {
    const hId = document.getElementById('staffHotelSelect').value;
    const date = document.getElementById('invoiceDate').value;
    ...
    try {
        const { data: invData, error: invErr } = await window.mySupabase.from('invoices').insert([{
            ...
            items: items, // 기존 JSON 타입 하위호환
            is_sent: false
        }]).select().single();

        if (invErr) throw invErr;

        // v35: invoice_items 테이블에도 insert (이중 안전장치)
