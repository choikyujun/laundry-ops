...
window.calculateAdminDashStats = async function() {
    ...
    // invoices 와 hotels 조인해서 contract_type 이 'unit' 또는 'special' 인 경우만 반영
    const { data: invData } = await window.mySupabase.from('invoices')
        .select('date, total_amount, hotel_id, hotels(name, contract_type)')
        .eq('factory_id', currentFactoryId); // 필터링 제거하여 모든 유형 포함
        
    if(invData) {
        console.log("DEBUG: invData received:", invData);
        invData.forEach(inv => {
            // 정액제는 이미 별도로 합산하므로, 여기서는 'fixed'가 아닌 경우만 합산
            if (inv.hotels && inv.hotels.contract_type === 'fixed') return;

            const hName = inv.hotels ? inv.hotels.name : '알수없음';
            console.log("DEBUG: Processing inv", inv.date, hName, inv.total_amount);
        ...
        console.log("DEBUG: Final hotelSales before render:", hotelSales);
    }
...
window.buildPrintAndSendReport = async function(hotelId, sDate, eDate) {
    ...
    const { data: allItems } = await window.mySupabase.from('hotel_item_prices').select('*').eq('hotel_id', hotelId).order('sort_order', { ascending: true }).order('created_at', { ascending: true });
    ...
    if (isSpecial) {
        ...
        orderedCats.forEach(cat => {
            ...
            // allItems 배열의 순서를 기준으로 정렬 (사용자가 입력/설정한 순서 유지)
            itemsInCat.sort((a, b) => {
                const idxA = allItems ? allItems.findIndex(ai => ai.name.toLowerCase() === a.name.toLowerCase()) : -1;
                const idxB = allItems ? allItems.findIndex(ai => ai.name.toLowerCase() === b.name.toLowerCase()) : -1;
