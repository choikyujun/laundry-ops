    ...
    if(filteredLogs.length > 0) {
        filteredLogs.forEach(log => {
            ...
            newHtml += `<tr>
                ...
                <td>${sentDate}</td>
                <td>
                    <button class="btn btn-neutral" style="background:var(--primary); color:white; padding:4px 8px; font-size:11px;" onclick="viewSentDetail('${hName}', '${log.period}', '${log.id}', false)">내역확인</button>
                    <button class="btn btn-danger" style="padding:4px 8px; font-size:11px; margin-left:5px;" onclick="deleteSentLog('${log.id}')">발송취소</button>
                </td>
            </tr>`;
        });
    } else {
...
window.openInvoiceModal = async function() {
    ...
    itemsToRender.forEach(item => {
        ...
        <tr>
            ...
            <td><input type="number" class="qty-input" value="0" min="0" oninput="calcTotal()" style="width:60px; padding:3px; text-align:center;"></td>
...
window.openInvoiceModal = async function() {
    ...
    itemsToRender.forEach(item => {
        ...
        <tr>
            ...
            <td><input type="number" class="inv-qty qty-input" value="0" min="0" oninput="calcTotal()" onkeydown="handleQtyKeydown(event)" style="width:60px; padding:3px; text-align:center;"></td>
            <td class="item-amount">0원</td>
        </tr>`;
    });
...
window.switchTab = async function(el, tabId) {
    ...
    const target = parent.querySelector('#tab_' + tabId);
    if(target) target.classList.add('active');
    else console.log("DEBUG: target 탭 없음, ID: tab_" + tabId);
    ...
    if(tabId === 'adminSent') { 
        console.log("DEBUG: adminSent 탭 로드 시도");
        if(typeof window.loadAdminSentList === 'function') {
            window.loadAdminSentList();
        } else {
            console.error("DEBUG: loadAdminSentList 함수가 정의되지 않음!");
        }
    }
};
...
window.calculateAdminDashStats = async function() { console.log("DEBUG: Function called");
    ...
    // 2. 정액제 매출 합산 (hotels)
    const { data: hotelData } = await window.mySupabase.from('hotels').select('contract_type, fixed_amount, created_at').eq('factory_id', currentFactoryId);
    let activeHotels = 0;
    if(hotelData) {
        hotelData.forEach(h => {
            activeHotels++;
...
// 1. 공장 정보 및 구독 상태 로드
    ...
    // 2. 매출 요약 계산 (DB 조회)
    console.log("DEBUG: Before calling runAdminStats");
    await window.runAdminStats();
    console.log("DEBUG: After calling runAdminStats");
...
window.calculateAdminDashStats = async function() { console.log("DEBUG: Function called");
    console.log("DEBUG: DEFINITION 1 (Global) started");
    const curMonth = document.getElementById('adminStatsMonth')?.value || getTodayString().substring(0, 7);
    const todayStr = getTodayString();
    ...
    // 1. 단가제 매출 (invoices)
    const { data: invData, error: invErr } = await window.mySupabase.from('invoices').select('date, total_amount, hotel_id, hotels(name)').eq('factory_id', currentFactoryId);
    console.log("DEBUG: invData", invData, "err", invErr);
    if(invData) {
        invData.forEach(inv => {
            if(inv.date === todayStr) todayRev += inv.total_amount;
    ...
    // 2. 정액제 매출 합산 (hotels)
    const { data: hotelData, error: hErr } = await window.mySupabase.from('hotels').select('name, contract_type, fixed_amount, created_at').eq('factory_id', currentFactoryId);
    console.log("DEBUG: hotelData", hotelData, "err", hErr);
    let activeHotels = 0;
    if(hotelData) {
        hotelData.forEach(h => {
            activeHotels++;
    ...
    console.log("DEBUG: hotelSales", hotelSales);
...
window.loadHotelCategoryList = async function() {
    ...
    const { data: hotel } = await window.mySupabase.from('hotels').select('contract_type').eq('id', hId).single();
    const isSpecial = hotel && hotel.contract_type === 'special';

    const { data: cats } = await window.mySupabase.from('hotel_categories').select('*').eq('hotel_id', hId).order('created_at');
    
    // [FORCE DELETE] If special, delete '기타' or '삭제' categories
    if (isSpecial && cats) {
        const toDelete = cats.filter(c => c.name === '기타' || c.name === '삭제');
...
window.viewInvoiceDetail = async function(id) {
    ...
    // [품목 정렬 및 카테고리 매핑] 설정된 단가표의 sort_order와 created_at을 기준으로 정렬
    const { data: priceList } = await window.mySupabase.from('hotel_item_prices')
        .select('name, sort_order, created_at, category_name')
        .eq('hotel_id', inv.hotel_id);
        
    if (priceList) {
        items.forEach(it => {
            const p = priceList.find(x => x.name === it.name);
            if(p && p.category_name) {
                it.category = p.category_name;
            }
        });
        items.sort((a, b) => {
            const pA = priceList.find(p => p.name === a.name);
            const pB = priceList.find(p => p.name === b.name);
            const orderA = pA && pA.sort_order != null ? pA.sort_order : 9999;
            const orderB = pB && pB.sort_order != null ? pB.sort_order : 9999;
            if (orderA !== orderB) return orderA - orderB;
            const timeA = pA ? new Date(pA.created_at).getTime() : 0;
            const timeB = pB ? new Date(pB.created_at).getTime() : 0;
            return timeA - timeB;
        });
    }
        ...
        Object.keys(grouped).forEach(cat => {
            ...
            <div style="break-inside: avoid; margin-bottom:5px; border:1px solid #cbd5e1;">
                ...
                <table style="width:100%; font-size:9px; border-collapse:collapse;">
                    <thead><tr style="background:#f8fafc;"><th style="border-right:1px solid #cbd5e1; padding:1px 2px;">품목</th><th style="border-right:1px solid #cbd5e1; padding:1px 2px;">단가</th><th style="border-right:1px solid #cbd5e1; padding:1px 2px;">수량</th><th style="padding:1px 2px;">금액</th></tr></thead>
                    <tbody>
                        ${grouped[cat].map(it => `<tr>
                            <td style="border-right:1px solid #cbd5e1; padding:1px 2px;">${it.name}</td>
                            <td style="border-right:1px solid #cbd5e1; padding:1px 2px; text-align:center;">${Number(it.price||0).toLocaleString()}</td>
                            <td style="border-right:1px solid #cbd5e1; padding:1px 2px; text-align:center;">${it.qty}</td>
                            <td style="padding:1px 2px; text-align:right;">₩ ${(Number(it.price||0) * Number(it.qty||0)).toLocaleString()}</td>
                        </tr>`).join('')}
                    </tbody>
                </table>
            </div>`;
                ...
                <tbody>
                    ${items.map(it => `
                        <tr>
                            ...
                            <td style="padding: 4px; border: 1px solid #cbd5e1; text-align: right;">${Number(it.price || 0).toLocaleString()}</td>
                            <td style="padding: 4px; border: 1px solid #cbd5e1; text-align: right;">${it.qty || 0}</td>
                            <td style="padding: 4px; border: 1px solid #cbd5e1; text-align: right;">₩ ${(Number(it.price || 0) * Number(it.qty || 0)).toLocaleString()}</td>
                        </tr>
                    `).join('')}
                </tbody>
                <tfoot>
                    <tr style="font-weight: 700; background: #e2e8f0;">
                        <td colspan="3" style="padding: 4px; border: 1px solid #cbd5e1; text-align: right;">공급가 합계</td>
                        <td style="padding: 4px; border: 1px solid #cbd5e1; text-align: right;">₩ ${actualSum.toLocaleString()}</td>
                    </tr>
                </tfoot>
            </table>
        </div>`;
